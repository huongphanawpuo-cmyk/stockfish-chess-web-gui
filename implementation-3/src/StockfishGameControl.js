...
export class StockfishGameControl extends GameControl {
	constructor(chessConsole, props) {
		...
		this.chessConsole.i18n.load({}).then(() => {
			this.$btnSetup = $(
				'<button type="button" id="btn-setup" class="btn btn-icon btn-primary" title="Setup / Import"><i class="fas fa-paste"></i></button>',
			);
			this.$btnClearAnnotations = $(
				'<button type="button" id="btn-clear-annotations" class="btn btn-icon btn-danger" title="Clear Annotations"><i class="fas fa-eraser"></i></button>',
			);
